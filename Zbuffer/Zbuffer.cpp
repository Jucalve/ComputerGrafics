...
const int win=0, w=1920, h=1080; //win 0 == true;
int GopMat=2; //0=scale; 1=trans; 2=rot;
int GopRot=1; //0=x; 1=y; 2=z;
std::string fl="Opiramide.vlf";
...
auto vzbuffer(zbuffer coef,int x,int y)
{
    double z = 0;
    if (coef.B != 0)
        z = (-coef.A * x - coef.C * y - coef.D) / coef.B;
    return z;
}
...
/***************************drawing*******************************/
  ...
  //cout<<fvsz<<endl; 
  cout<<co[504].x <<", " <<co[504].y <<", "<<co[504].z <<", " <<endl;

  co=raster->Transf(0, ang, GopRot, co, Nv, (float)unidadx, (float)unidadx, (float)unidadx); 
  cout<<co[504].x <<", " <<co[504].y <<", "<<co[504].z <<", " <<endl;

  co=raster->Transf(1, ang, GopRot, co, Nv, mitadx, mitady, 1);
  cout<<co[504].x <<", " <<co[504].y <<", "<<co[504].z <<", " <<endl;
  ...
  for(i=0;i<fvsz; i++){//XY
    ...
    auxZbuffer.clear();
      ...
      raster->ZBufferFunction(auxZbuffer[0],auxZbuffer[1],auxZbuffer[2],auxZbuffer[3],auxZbuffer[4],auxZbuffer[5],auxZbuffer[6],auxZbuffer[7],auxZbuffer[8],(1*rand()*100)%255,((1*rand()*100)%255),((1*rand()*100)%255));
  }
  cout<<"sali"<<endl;
  co=raster->Transf(1, ang, GopRot, co, Nv, (float)(-mitadx), (float)(-mitady), 0); 

  co=raster->Transf(1, ang, GopRot, co, Nv, 0, (float)(mitady*3), (float)(mitadx)); 
  for(i=0;i<fvsz; i++){//ZY
     auxZbuffer.clear();
      for(j=0; j<3; j++){
        ...
        auxZbuffer.push_back((int)y1);
        auxZbuffer.push_back((int)co[ carasv[i][j] ].x);
        //218, 165, 32);  //dorado
      }
     raster->ZBufferFunction(auxZbuffer[0],auxZbuffer[1],auxZbuffer[2],auxZbuffer[3],auxZbuffer[4],auxZbuffer[5],auxZbuffer[6],auxZbuffer[7],auxZbuffer[8],(1*rand()*100)%255,((1*rand()*100)%255),((1*rand()*100)%255));
